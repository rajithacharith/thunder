...
export class ThunderMFASetup {
  ...
  private async createOrGetTestUser(adminToken: string): Promise<string> {
    ...
    const availableTypes = (typesData.schemas ?? typesData.types ?? typesData.userTypes ?? []) as Array<{
      name?: string;
      ouId?: string;
    }>;
    const personType = availableTypes.find(
      userType => typeof userType.name === "string" && userType.name.toLowerCase() === "person"
    );
