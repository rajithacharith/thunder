...
func (suite *InitTestSuite) TestInitialize() {
	mgtService, otpService, err := Initialize(suite.mux, suite.mockJWTService)
	suite.NoError(err)
...
func (suite *InitTestSuite) TestRegisterRoutes_ListEndpoint() {
	_, _, err := Initialize(suite.mux, suite.mockJWTService)
	suite.NoError(err)
...
`
	...
	_, _, err = Initialize(mux, suite.mockJWTService)
	suite.Error(err)
