...
func Initialize(mux *http.ServeMux) (IDPServiceInterface, error) {
	...
	if immutableresource.IsImmutableModeEnabled() {
		...
		if fileBasedStore, ok := idpStore.(*idpFileBasedStore); ok {
			...
			return nil, fmt.Errorf("invalid store type for immutable resources")
		}
		...
		if err := loader.LoadResources(); err != nil {
			return nil, fmt.Errorf("failed to load identity provider resources: %w", err)
		}
	}
