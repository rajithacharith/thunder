...
var (
	// queryGetUserSchemaCount retrieves the total count of user types.
	...
	// queryGetUserSchemaList retrieves a paginated list of user types.
	queryGetUserSchemaList = dbmodel.DBQuery{
		ID: "ASQ-USER_SCHEMA-002",
		Query: `SELECT ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, SYSTEM_ATTRIBUTES FROM USER_TYPES ` +
			`WHERE DEPLOYMENT_ID = $3 ORDER BY NAME LIMIT $1 OFFSET $2`,
	}

	// queryCreateUserSchema creates a new user type.
	queryCreateUserSchema = dbmodel.DBQuery{
		ID: "ASQ-USER_SCHEMA-003",
		Query: `INSERT INTO USER_TYPES ` +
			`(ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, SCHEMA_DEF, SYSTEM_ATTRIBUTES, DEPLOYMENT_ID) ` +
			`VALUES ($1, $2, $3, $4, $5, $6, $7)`,
	}

	// queryGetUserSchemaByID retrieves a user type by its ID.
	queryGetUserSchemaByID = dbmodel.DBQuery{
		ID: "ASQ-USER_SCHEMA-004",
		Query: `SELECT ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, SCHEMA_DEF, ` +
			`SYSTEM_ATTRIBUTES FROM USER_TYPES WHERE ID = $1 AND DEPLOYMENT_ID = $2`,
	}

	// queryGetUserSchemaByName retrieves a user type by its name.
	queryGetUserSchemaByName = dbmodel.DBQuery{
		ID: "ASQ-USER_SCHEMA-005",
		Query: `SELECT ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, SCHEMA_DEF, ` +
			`SYSTEM_ATTRIBUTES FROM USER_TYPES WHERE NAME = $1 AND DEPLOYMENT_ID = $2`,
	}

	// queryUpdateUserSchemaByID updates a user type by its ID.
	...
	// queryDeleteUserSchemaByID deletes a user type by its ID.
	queryDeleteUserSchemaByID = dbmodel.DBQuery{
		ID:    "ASQ-USER_SCHEMA-007",
		Query: `DELETE FROM USER_TYPES WHERE ID = $1 AND DEPLOYMENT_ID = $2`,
	}
)

// buildGetUserSchemaListByOUIDsQuery dynamically builds a query to retrieve user types
...
func buildGetUserSchemaListByOUIDsQuery(ouIDs []string) dbmodel.DBQuery {
	...
	if n == 0 {
		return dbmodel.DBQuery{
			...
			PostgresQuery: `SELECT ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, ` +
				`SYSTEM_ATTRIBUTES FROM USER_TYPES ` +
				`WHERE 1=0 AND DEPLOYMENT_ID = $1 ORDER BY NAME LIMIT $2 OFFSET $3`,
			SQLiteQuery: `SELECT ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, ` +
				`SYSTEM_ATTRIBUTES FROM USER_TYPES ` +
...
// buildGetUserSchemaCountByOUIDsQuery dynamically builds a query to count user types
...
func buildGetUserSchemaCountByOUIDsQuery(ouIDs []string) dbmodel.DBQuery {
	...
	if n == 0 {
		return dbmodel.DBQuery{
			...
			PostgresQuery: `SELECT COUNT(*) AS total FROM USER_TYPES ` +
				`WHERE 1=0 AND DEPLOYMENT_ID = $1`,
			SQLiteQuery: `SELECT COUNT(*) AS total FROM USER_TYPES ` +
...
func buildGetDisplayAttributesByNamesQuery(names []string) dbmodel.DBQuery {
	...
	if n == 0 {
		return dbmodel.DBQuery{
			...
			PostgresQuery: `SELECT NAME, SYSTEM_ATTRIBUTES FROM USER_TYPES WHERE 1=0 AND DEPLOYMENT_ID = $1`,
			SQLiteQuery:   `SELECT NAME, SYSTEM_ATTRIBUTES FROM USER_TYPES WHERE 1=0 AND DEPLOYMENT_ID = ?`,
		}
	}
	...
	return dbmodel.DBQuery{
		ID: "ASQ-USER_SCHEMA-010",
		PostgresQuery: `SELECT NAME, SYSTEM_ATTRIBUTES FROM USER_TYPES ` +
			`WHERE NAME IN (` + pgInClause + `) AND DEPLOYMENT_ID = ` + pgDeploymentID,
		SQLiteQuery: `SELECT NAME, SYSTEM_ATTRIBUTES FROM USER_TYPES ` +
