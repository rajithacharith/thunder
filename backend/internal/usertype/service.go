...
// Package usertype handles the user schema management operations.
package usertype
...
func (us *userSchemaService) GetUserSchemaList(ctx context.Context, limit, offset int) (
	*UserSchemaListResponse, *serviceerror.ServiceError) {
	...
	accessible, svcErr := us.getAccessibleResources(ctx, security.ActionListUserTypes)
	if svcErr != nil {
		return nil, svcErr
	}
...
func (us *userSchemaService) CreateUserSchema(
	...
	if svcErr := us.checkUserSchemaAccess(
		ctx, security.ActionCreateUserType, request.OUID); svcErr != nil {
		return nil, svcErr
	}
...
func (us *userSchemaService) GetUserSchema(
	...
	// Check authorization
	if svcErr := us.checkUserSchemaAccess(
		ctx, security.ActionReadUserType, userSchema.OUID); svcErr != nil {
		return nil, svcErr
	}
...
func (us *userSchemaService) UpdateUserSchema(ctx context.Context, schemaID string, request UpdateUserSchemaRequest) (
	*UserSchema, *serviceerror.ServiceError) {
	...
	// Check authorization
	if svcErr := us.checkUserSchemaAccess(
		ctx, security.ActionUpdateUserType, existingSchema.OUID); svcErr != nil {
		return nil, svcErr
	}
	...
	if request.OUID != existingSchema.OUID {
		if svcErr := us.checkUserSchemaAccess(
			ctx, security.ActionUpdateUserType, request.OUID); svcErr != nil {
			return nil, svcErr
		}
	}
...
func (us *userSchemaService) DeleteUserSchema(ctx context.Context, schemaID string) *serviceerror.ServiceError {
	...
	if err != nil {
		if errors.Is(err, ErrUserSchemaNotFound) {
			...
			if svcErr := us.checkUserSchemaAccess(
				ctx, security.ActionDeleteUserType, ""); svcErr != nil {
				return svcErr
			}
	...
	if svcErr := us.checkUserSchemaAccess(
		ctx, security.ActionDeleteUserType, existingSchema.OUID); svcErr != nil {
		return svcErr
	}
...
func (us *userSchemaService) checkUserSchemaAccess(
	...
	allowed, svcErr := us.authzService.IsActionAllowed(ctx, action,
		&sysauthz.ActionContext{ResourceType: security.ResourceTypeUserType, OUID: ouID})
	if svcErr != nil {
...
func (us *userSchemaService) getAccessibleResources(
	...
	accessible, svcErr := us.authzService.GetAccessibleResources(
		ctx, action, security.ResourceTypeUserType)
	if svcErr != nil {
		return nil, &ErrorInternalServerError
	}
