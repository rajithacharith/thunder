...
func TestBuildGetUserSchemaListByOUIDsQuery(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			name:  "Empty OUIDs",
			ouIDs: []string{},
			wantPG: `SELECT ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, ` +
				`SYSTEM_ATTRIBUTES FROM USER_TYPES ` +
				`WHERE 1=0 AND DEPLOYMENT_ID = $1 ORDER BY NAME LIMIT $2 OFFSET $3`,
			wantSQLite: `SELECT ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, ` +
				`SYSTEM_ATTRIBUTES FROM USER_TYPES ` +
...
func TestBuildGetUserSchemaCountByOUIDsQuery(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			name:  "Empty OUIDs",
			ouIDs: []string{},
			wantPG: `SELECT COUNT(*) AS total FROM USER_TYPES ` +
				`WHERE 1=0 AND DEPLOYMENT_ID = $1`,
			wantSQLite: `SELECT COUNT(*) AS total FROM USER_TYPES ` +
...
func TestBuildGetDisplayAttributesByNamesQuery(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			name:       "Empty names",
			ouIDs:      []string{},
			wantPG:     `SELECT NAME, SYSTEM_ATTRIBUTES FROM USER_TYPES WHERE 1=0 AND DEPLOYMENT_ID = $1`,
			wantSQLite: `SELECT NAME, SYSTEM_ATTRIBUTES FROM USER_TYPES WHERE 1=0 AND DEPLOYMENT_ID = ?`,
		},
		{
			name:  "Single name",
			ouIDs: []string{"SchemaA"},
			wantPG: `SELECT NAME, SYSTEM_ATTRIBUTES FROM USER_TYPES ` +
				`WHERE NAME IN ($1) AND DEPLOYMENT_ID = $2`,
			wantSQLite: `SELECT NAME, SYSTEM_ATTRIBUTES FROM USER_TYPES ` +
