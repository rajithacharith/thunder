...
// AddGroupMembers must succeed for declarative groups — member assignments are always DB-backed.
func (suite *DeclarativeModeGroupServiceTestSuite) TestAddGroupMembers_AllowedForDeclarativeGroup() {
	...
	suite.store.On("AddGroupMembers", mock.Anything, "group-1", normalizedMembers).Return(nil)

	grp, err := suite.service.AddGroupMembers(suite.ctx, "group-1", []Member{{ID: "user-1", Type: MemberTypeUser}})

	suite.Nil(err)
	assert.NotNil(suite.T(), grp)
	suite.store.AssertCalled(suite.T(), "AddGroupMembers", mock.Anything, "group-1", normalizedMembers)
}

// RemoveGroupMembers must succeed for declarative groups — member assignments are always DB-backed.
func (suite *DeclarativeModeGroupServiceTestSuite) TestRemoveGroupMembers_AllowedForDeclarativeGroup() {
	grpDAO := GroupDAO{ID: "group-1", Name: "Admins", OUID: "ou-1"}
	normalizedMembers := []Member{{ID: "user-1", Type: memberTypeEntity}}
	...
	suite.entityService.On("GetEntitiesByIDs", mock.Anything, []string{"user-1"}).
		Return([]entity.Entity{{ID: "user-1", Category: entity.EntityCategoryUser}}, nil)
	suite.store.On("RemoveGroupMembers", mock.Anything, "group-1", normalizedMembers).Return(nil)

	grp, err := suite.service.RemoveGroupMembers(suite.ctx, "group-1", []Member{{ID: "user-1", Type: MemberTypeUser}})

	suite.Nil(err)
	assert.NotNil(suite.T(), grp)
	suite.store.AssertCalled(suite.T(), "RemoveGroupMembers", mock.Anything, "group-1", normalizedMembers)
}
