...
func (c *compositeGroupStore) GetGroupMembers(
	...
	members, limitExceeded, err := declarativeresource.CompositeMergeListHelperWithLimit(
		capCount(c.dbStore.GetGroupMemberCount),
		capCount(c.fileStore.GetGroupMemberCount),
		func(count int) ([]Member, error) { return c.dbStore.GetGroupMembers(ctx, groupID, count, 0) },
		func(count int) ([]Member, error) { return c.fileStore.GetGroupMembers(ctx, groupID, count, 0) },
		mergeMembers,
		limit,
		offset,
		serverconst.MaxCompositeStoreRecords,
...
func (c *compositeGroupStore) GetGroupsByOrganizationUnitCount(ctx context.Context, oUID string) (int, error) {
	...
	groups, limitExceeded, err := declarativeresource.CompositeMergeListHelperWithLimit(
		capCount(c.dbStore.GetGroupsByOrganizationUnitCount),
		capCount(c.fileStore.GetGroupsByOrganizationUnitCount),
		func(count int) ([]GroupBasicDAO, error) {
			return c.dbStore.GetGroupsByOrganizationUnit(ctx, oUID, count, 0)
		},
		func(count int) ([]GroupBasicDAO, error) {
			return c.fileStore.GetGroupsByOrganizationUnit(ctx, oUID, count, 0)
		},
		mergeGroupBasicDAOs,
		serverconst.MaxCompositeStoreRecords+1,
		0,
