...
var (
	queryUpsertRequestState = dbmodel.DBQuery{
		...
		Query: `INSERT INTO "OPENID4VP_REQUEST_STATE" ` +
			`(STATE, DEPLOYMENT_ID, DEFINITION_ID, NONCE, EPHEMERAL_KEY, CLIENT_ID, REQUEST_URI, ` +
			...
			`REQUEST_URI = EXCLUDED.REQUEST_URI, STATUS = EXCLUDED.STATUS, ` +
			`RESULT = EXCLUDED.RESULT, FAILURE_REASON = EXCLUDED.FAILURE_REASON, ` +
			`EXPIRY_TIME = EXCLUDED.EXPIRY_TIME`,
	}
	queryGetRequestState = dbmodel.DBQuery{
		ID: "OVPQ-RS-02",
		Query: `SELECT STATE, DEFINITION_ID, NONCE, EPHEMERAL_KEY, CLIENT_ID, REQUEST_URI, ` +
