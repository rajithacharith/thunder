...
No custom React component is needed for signup. When self-registration is enabled, the ThunderID-hosted sign-in page automatically renders a **Sign Up** link.

**Two conditions must both be met:**

**1. The application must have registration enabled.**

Use ~thunderid_update_application~ to set ~isRegistrationFlowEnabled~ and point it at a registration flow. Also add the user type name to ~allowedUserTypes~:
...
Use ~thunderid_list_user_types~ to find user types. The user type(s) listed in ~allowedUserTypes~ above must have ~allowSelfRegistration: true~. Without this, the sign-in page will not show the Sign Up link even if the application has registration enabled.

Once both conditions are met, the hosted sign-in page shows the Sign Up link automatically — no changes to your React app are required.
...
User attributes are only available in the React SDK if the application's inbound OAuth config explicitly includes them. Use the ~thunderid_update_application~ tool to set this on the application:
...
The OAuth scopes requested at login must also cover the attributes you need (e.g. ~openid profile email~). Attributes not listed in ~userAttributes~ above will not appear in the ID token or the ~/oauth2/userinfo~ response regardless of requested scopes.
...
Use ~thunderid_list_applications~ to find your application and copy its UUID. If you need a new application, use ~thunderid_create_application~ and note the returned UUID.
...
Replace ~<SignInButton />~ with the ~<SignIn />~ component. ~<SignIn />~ renders the app-native authentication UI inline — no browser redirect occurs and no separate sign-in page or route is needed.

The ~<SignIn />~ component handles the entire auth flow (username/password fields, step-up prompts, etc.) within your app itself.
...
If you previously had a dedicated sign-in route (e.g. ~<Route path="/signin" element={<SignInPage />} />~) just to host the ~<SignInButton />~, remove that route entirely and render ~<SignIn />~ directly in your main layout.
...
If you were calling ~signIn()~ immediately after ~signOut()~ to return to the login screen, remove that call — ~<SignIn />~ handles re-rendering automatically when the user is signed out.
...
The signup flow is server-driven. When the ~<SignUp>~ component mounts, it calls the ~/flow/execute~ API to initialize a ~REGISTRATION~ flow. The server responds with a list of **components** (fields, buttons, dividers, etc.) that describe what to render. On each form submission the same API is called again with the user's inputs until ~flowStatus === 'COMPLETE'~.
...
| Scenario | Behaviour without ~applicationId~ |
|---|---|
| Branding (colours, logo) | Falls back to org-level defaults |
| Signup flow initialization | SDK reads ~applicationId~ from the URL query param as a fallback, otherwise omits it from the request |
...
Alternatively you can pass ~applicationId~ as a URL query param (~?applicationId=...~) and the SDK will pick it up automatically — useful when the signup page is shared across multiple apps.

### Create the signup page/route

Point users to a dedicated page that renders the ~<SignUp>~ component. Configure ~signUpUrl~ in the provider to that page:
...
The component renders whatever fields the server sends (username, email, password, etc.) and handles all flow steps automatically.
...
~~~tsx
<SignUp
  afterSignUpUrl="/welcome"           // auto-redirect on success (default behaviour)
...
If you need custom redirect logic (e.g. Next.js ~router.push~), set ~shouldRedirectAfterSignUp={false}~ and handle it in ~onComplete~.
...
| ~handleSubmit~ | ~(component, data?) => Promise<void>~ | Advance the flow — pass the button component and current ~values~ |
| ~validateForm~ | ~() => {fieldErrors, isValid}~ | Manually trigger validation |

### Key rules

- **Use ~component.ref~ as the field key**, not ~component.id~. The SDK maps input names to ~ref~ when building the ~inputs~ payload sent to the server.
- **Social / OAuth signup** (Google, GitHub, etc.) is handled automatically via a popup — your render-prop UI does not need to handle ~type: 'REDIRECTION'~ responses.
- **Passkey registration** is also handled automatically when ~additionalData.passkeyCreationOptions~ is present in the flow response.
- The flow can have **multiple steps** (e.g. email → OTP → password). The ~components~ array updates after each successful submission — your UI re-renders the new step automatically.
