...
func Initialize(
...
) (UserSchemaServiceInterface, error) {
	...
	if config.GetThunderRuntime().Config.ImmutableResources.Enabled {
		...
		if !ok {
			return nil, fmt.Errorf("failed to assert userSchemaStore to *userSchemaFileBasedStore")
		}
		...
		if err := loader.LoadResources(); err != nil {
			return nil, fmt.Errorf("failed to load user schema resources: %w", err)
		}
	}

	userSchemaHandler := newUserSchemaHandler(userSchemaService)
	registerRoutes(mux, userSchemaHandler)
	return userSchemaService, nil
}
