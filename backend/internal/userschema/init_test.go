...
func (suite *InitTestSuite) TestInitialize() {
	...
	service, err := Initialize(suite.mux, suite.mockOUService)
	assert.NoError(suite.T(), err)
...
func (suite *InitTestSuite) TestRegisterRoutes_ListEndpoint() {
	...
	_, err = Initialize(suite.mux, suite.mockOUService)
	assert.NoError(suite.T(), err)
...
func TestInitialize_Standalone(t *testing.T) {
	...
	service, err := Initialize(mux, mockOUService)
	assert.NoError(t, err)
...
`
	...
	_, err = Initialize(mux, mockOUService)
	assert.Error(t, err)
