...
var (
	...
	queryCreateUserSchema = dbmodel.DBQuery{
		...
		Query: `INSERT INTO USER_SCHEMAS ` +
			`(SCHEMA_ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, SCHEMA_DEF, SYSTEM_ATTRIBUTES, DEPLOYMENT_ID) ` +
	...
	queryGetUserSchemaByID = dbmodel.DBQuery{
		...
		Query: `SELECT SCHEMA_ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, SCHEMA_DEF, ` +
			`SYSTEM_ATTRIBUTES FROM USER_SCHEMAS WHERE SCHEMA_ID = $1 AND DEPLOYMENT_ID = $2`,
	}
...
func buildGetUserSchemaListByOUIDsQuery(ouIDs []string) dbmodel.DBQuery {
	...
	if n == 0 {
		return dbmodel.DBQuery{
			...
			PostgresQuery: `SELECT SCHEMA_ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, ` +
				`SYSTEM_ATTRIBUTES FROM USER_SCHEMAS ` +
				`WHERE 1=0 AND DEPLOYMENT_ID = $1 ORDER BY NAME LIMIT $2 OFFSET $3`,
			SQLiteQuery: `SELECT SCHEMA_ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, ` +
				`SYSTEM_ATTRIBUTES FROM USER_SCHEMAS ` +
