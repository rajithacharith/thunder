...
var (
	...
	queryGetUserSchemaList = dbmodel.DBQuery{
		...
		Query: `SELECT ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, SYSTEM_ATTRIBUTES FROM USER_SCHEMAS ` +
	...
	queryGetUserSchemaByID = dbmodel.DBQuery{
		...
		Query: `SELECT ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, SCHEMA_DEF, ` +
			`SYSTEM_ATTRIBUTES FROM USER_SCHEMAS WHERE ID = $1 AND DEPLOYMENT_ID = $2`,
	}
	...
	queryGetUserSchemaByName = dbmodel.DBQuery{
		...
		Query: `SELECT ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, SCHEMA_DEF, ` +
			`SYSTEM_ATTRIBUTES FROM USER_SCHEMAS WHERE NAME = $1 AND DEPLOYMENT_ID = $2`,
	}
	...
	if n == 0 {
		return dbmodel.DBQuery{
			...
			PostgresQuery: `SELECT ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, ` +
				`SYSTEM_ATTRIBUTES FROM USER_SCHEMAS ` +
