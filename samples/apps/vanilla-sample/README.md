...
`next dev --experimental-https` generates and trusts a local certificate itself, so no shipped `server.key`/`server.cert` is required. If your machine can't install the local certificate authority (e.g. no sudo), the dev server falls back to plain HTTP; passkeys and some social login callbacks require HTTPS, so grant the prompt if you plan to try those.
