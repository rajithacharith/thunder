---
title: Configuration
description: ThunderIDConfig options for the iOS SDK
---

# Configuration

`ThunderIDConfig` is the configuration struct passed to `ThunderClient.initialize(config:storage:)` and to the `.thunderIDProvider(config:)` view modifier.
...
Pass your implementation to `ThunderIDConfig(storage:)` or to `ThunderClient.initialize(config:storage:)`.
