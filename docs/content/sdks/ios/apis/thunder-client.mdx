---
title: ThunderClient
description: Core authentication client for the ThunderID iOS SDK
---

# `ThunderClient`

`ThunderClient` is the core authentication client in the `ThunderID` package. It manages the full authentication lifecycle: initialization, sign-in (both app-native and redirect-based), session, token management, and user profile operations.

When you use the `ThunderSwiftUI` package, a `ThunderClient` instance is created and managed automatically. You can access it via `ThunderState.client` from any view.
...
:::note
When using `ThunderSwiftUI`, you do not need to create a `ThunderClient` directly. Apply `.thunderIDProvider(config:)` to your root view and access the client through `@EnvironmentObject var state: ThunderState`.
:::
...
| :--- | :--- | :---: | :--- |
| `config` | `ThunderIDConfig` | ✅ | SDK configuration. See [Configuration](/docs/next/sdks/ios/apis/configuration). |
