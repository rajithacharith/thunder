---
title: UserProfile
description: Editable user profile form
---

# `UserProfile`

The `UserProfile` component renders an editable profile form. It loads the user's current profile from `/scim2/Me` when it appears, presents editable fields for `displayName` and `phoneNumbers`, and saves changes via `ThunderClient.updateUserProfile`.
