---
title: SignInButton
description: Button that triggers the sign-in flow
---

# `SignInButton`

The `SignInButton` component renders a pre-styled button that you can use to trigger your sign-in flow. It reads loading state from `ThunderState` and disables itself automatically while an operation is in progress.
