---
title: SignUp
description: Embedded registration form driven by the Flow Execution API
---
...
Use `onComplete` to navigate away from the registration screen after a successful sign-up. `ThunderState.user` is updated automatically before this callback fires.
...
`SignUpState` exposes the same properties and methods as `SignInState`. See [SignIn — SignInState Properties](/docs/next/sdks/ios/apis/components/sign-in#signinstate-properties) for the full reference.
