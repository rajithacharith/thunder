...
`SignUpState` has the same shape as [`SignInState`](/docs/next/sdks/android/apis/components/sign-in#signinstate-properties): `inputs`, `actions`, `isLoading`, `error`, `binding(name)`, and `submit(actionId)`.
