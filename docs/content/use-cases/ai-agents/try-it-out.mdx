...
2. **Create the `wayfinder` resource server.**
   ...
   Invoke the **Resource Management API** again. Set the identifier to `http://localhost:8787/mcp` and the handle to `wayfinder`:
...
5. **Create the `Recommender` role.**
   ...
   Grant it `wayfinder:booking:recommend` on `wayfinder`. This role will be assigned to the Wayfinder Concierge, not to a user:
...
12. **Create the `Wayfinder Agent Authentication Flow`.**
    ...
    Navigate to **Flows** and create a flow that authenticates the user with username and password, then shows a consent screen listing the `wayfinder:booking:*` permissions the agent is requesting. Attach the flow to the `WAYFINDER-CONCIERGE` agent.
