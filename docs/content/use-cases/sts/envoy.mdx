...
## Configure Fine-Grained Authorization in Envoy
...
Scope-based authorization adds a second layer of control. <ProductName /> embeds scopes in tokens based on the roles assigned to an application. Envoy then checks those scopes on every request and rejects tokens that do not carry the required permissions. This lets you enforce fine-grained access control — for example, restricting a read endpoint to tokens with `resources-api:resource:read` — without changing the upstream service.
