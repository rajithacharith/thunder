...
server or security bypass is required. The subcommand never invents an admin password
on its own, so pass one explicitly with `--admin-password`:
...
2. Log in with the admin credentials from Step 2 (username `admin`, password `admin` if you used the example above)
...
Open DBeaver and view the `ENTITY` table in the Entity DB to confirm the new user was created.
