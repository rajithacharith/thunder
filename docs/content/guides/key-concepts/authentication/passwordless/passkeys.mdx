...
1. **Thunder Gate (Hosted UI)**: Use Thunder's hosted authentication pages (`gate`) by configuring passkeys in your application settings—no custom UI or API calls needed
...
1. **Create an application** in the Thunder Console (or via the Application API).
...
This approach requires **no custom UI development** or direct WebAuthn API calls from your application. Thunder Gate (`gate`) handles:
