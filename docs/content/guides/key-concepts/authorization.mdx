...
This mechanism prevents a token issued for one service from being reused at another. Each non-CIBA access token targets exactly one resource server. Requested scopes are filtered to only those defined on the targeted resource server, and app or agent tokens are further limited to permissions assigned to the issuing app or agent.
