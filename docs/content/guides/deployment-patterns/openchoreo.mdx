...
<ProductName /> signs tokens with two key pairs, one RSA and one ECDSA. The container image ships no key material, so generate both pairs before the first deployment. Self-signed pairs are fine here; these keys sign tokens, they do not terminate TLS.
...
Push the sensitive values, together with the signing key files, to the platform secret store as properties of one secret per environment. Scalar values become container environment variables; the certificate and key properties are mounted as files under `config/certs/`. The following example uses the OpenBao instance installed with the OpenChoreo quick-start:
...
The `ResourceType` renders an `ExternalSecret` that extracts every property at the store path. Scalar values are injected into the container environment, where <ProductName /> resolves the `{{.VAR}}` placeholders in its configuration at startup; the certificate and key properties are mounted as files under `config/certs/`.
