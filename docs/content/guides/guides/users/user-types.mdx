...
Every user in Thunder has a user type that represents a category of users in your organization. The user type determines:
...
Thunder ships with two default user types. Learn more about the schemas of these default user types in [User Schema Reference](./user-schema-reference).
...
1. Navigate to **User Types** in the Thunder Console and click **Create User Type**.
...
5. Set the attributes that the user type should have, including data types and constraint modifiers. See [User Schema Reference](./user-schema-reference) for details on available settings and attribute options.
...
- [User Schema Reference](./users/user-schema-reference) - Schema settings, attribute types, and constraint modifiers
- [Manage Users](../users/manage-users) - Create and manage users of each type
- [Organization Units](../organization-units) - Understand the OU structure that binds user types.
- [Flows](./flows) - Configure registration flows for self-service sign-up
