...
sidebar_position: 5
persona: iam
description: Complete reference for all Starter Templates, Widgets, Steps, Components, and Executors available in the Thunder Console Flow Builder.
---
...
Steps are individual View nodes — the UI screen only, with no pre-wired executors. Use a Step when you need a custom combination not covered by a Widget.
...
| **Blank View** (with attribute fields) | Attribute Collector | User must be authenticated | Use when collecting profile attributes required by the user schema. The executor checks the user's existing attributes and prompts only for what is missing. |
