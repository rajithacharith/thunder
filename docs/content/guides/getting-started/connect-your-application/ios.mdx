...
<TutorialHeroItem icon={<Package />}>Install the <code>ThunderSwiftUI</code> Swift package</TutorialHeroItem>
...
When prompted to choose package products, select **ThunderSwiftUI**. This includes the core `ThunderID` client as a dependency.
...
```
io.thunder.b2c://callback
```

Add the same value as an **Allowed Post-Logout Redirect URI**:

```
io.thunder.b2c://logout
```

## Initialize the SDK

Open your app's entry point (the file that conforms to `App`) and apply the `.thunderIDProvider(config:)` modifier to your root view. This injects a `ThunderState` environment object into all child views.
